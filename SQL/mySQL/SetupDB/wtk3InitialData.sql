...
INSERT INTO `wtkLookups` (`LookupType`, `LookupValue`, `LookupDisplay`)
 VALUES
   ...
   ('FailCode','CFer','Failed Cloudflare Turnstile challenge'),
   ('FailCode','WP','Wordpress exploit attempt'),
   ('PagePath', '/wtk/', 'WTK'),
   ...
   ('AbuseIPCode', '4', 'DDoS Attack'),
   ('AbuseIPCode', '5', 'FTP Brute-Force'),
   ('AbuseIPCode', '6', 'Ping of Death'),
   ('AbuseIPCode', '7', 'Phishing'),
   ('AbuseIPCode', '8', 'Fraud VoIP'),
   ('AbuseIPCode', '9', 'Open Proxy'),
   ('AbuseIPCode', '10', 'Web Spam'),
   ('AbuseIPCode', '11', 'Email Spam'),
   ('AbuseIPCode', '12', 'Blog Spam'),
   ('AbuseIPCode', '13', 'VPN IP'),
   ('AbuseIPCode', '14', 'Port Scan'),
   ('AbuseIPCode', '15', 'Hacking'),
   ('AbuseIPCode', '16', 'SQL Injection'),
   ('AbuseIPCode', '17', 'Spoofing'),
   ('AbuseIPCode', '18', 'Brute-Force'),
   ('AbuseIPCode', '19', 'Bad WebBot'),
   ('AbuseIPCode', '20', 'Exploited Host'),
   ('AbuseIPCode', '21', 'Web App Attack'),
   ('AbuseIPCode', '22', 'SSH'),
   ('AbuseIPCode', '23', 'IoT Targeted');
...
INSERT INTO `wtkWidget` (`UID`, `WidgetName`, `SecurityLevel`, `WidgetType`, `ChartType`, `WidgetColor`, `SkipFooter`, `WidgetDescription`, `WidgetSQL`, `WidgetURL`, `WindowModal`) VALUES
...
(2, 'Last 10 Users', 30, 'List', NULL, NULL, 'Y', 'Last 10 unique users that accessed website.', 'SELECT wu.`FirstName` AS `User`,\r\n    DATE_FORMAT(h.`AddDate`, \'%c/%e/%y at %k:%i\') AS `LastAccess`,\r\n    h.`PageURL`\r\nFROM `wtkUserHistory` h\r\n  INNER JOIN (\r\n    SELECT MAX(`UID`) AS `MaxUID`\r\n    FROM `wtkUserHistory`\r\n    GROUP BY `UserUID`\r\n  ) latest ON h.`UID` = latest.`MaxUID`\r\n  INNER JOIN `wtkUsers` wu ON wu.`UID` = h.`UserUID`\r\nWHERE wu.`DelDate` IS NULL\r\nORDER BY h.`UID` DESC LIMIT 10', NULL, 'N'),
(3, 'New Users - 7 Days', 30, 'Count', 'Area', 'danger-gradient', NULL, 'how many users signed up in the last 7 days', 'SELECT COUNT(`UID`)\r\nFROM `wtkUsers`\r\nWHERE `DelDate` IS NULL AND `AddDate` > (NOW() - INTERVAL 7 DAY)\r\n AND `SecurityLevel` < 80', NULL, 'N'),
(4, 'Revenue Today', 80, 'Count', 'Area', 'success-gradient', NULL, NULL, 'SELECT CONCAT(\'$\', FORMAT(COALESCE(SUM(`GrossAmount`), 0),0)) AS `Income`\r\nFROM `wtkRevenue`\r\nWHERE DATE_FORMAT(`AddDate`, \'%Y-%m-%d\') = CURRENT_DATE\r\n   AND `PaymentStatus` IN (\'paid\',\'succeeded\',\'confirmed\',\'Authorized\')', '/admin/revenueList', 'N'),
(5, 'Revenue This Week', 80, 'Count', 'Area', 'info-gradient', NULL, 'income from the last 7 days', 'SELECT CONCAT(\'$\', FORMAT(COALESCE(SUM(`GrossAmount`), 0),0)) AS `Income`\r\nFROM `wtkRevenue`\r\nWHERE `AddDate` > (CURRENT_DATE - INTERVAL 7 DAY)\r\n  AND `PaymentStatus` IN (\'paid\',\'succeeded\',\'confirmed\',\'Authorized\')', '/admin/revenueList', 'N'),
(6, 'Revenue This Month', 80, 'Count', 'Area', 'danger-gradient', NULL, 'income from the last 30 days', 'SELECT CONCAT(\'$\', FORMAT(COALESCE(SUM(`GrossAmount`), 0),0)) AS `Income`\r\nFROM `wtkRevenue`\r\nWHERE `AddDate` > (CURRENT_DATE - INTERVAL 30 DAY)\r\n  AND `PaymentStatus` IN (\'paid\',\'succeeded\',\'confirmed\',\'Authorized\')', '/admin/revenueList', 'N'),
(7, 'Daily Income', 80, 'Chart', 'Line', 'danger-gradient', NULL, 'last 7 days income summaries', 'SELECT DATE_FORMAT(`AddDate`, \'%b %D (%a)\') AS `Day`,\r\n   COALESCE(SUM(`GrossAmount`), 0) AS `Income`\r\n FROM `wtkRevenue`\r\nWHERE `PaymentStatus` IN (\'paid\',\'succeeded\',\'confirmed\',\'Authorized\')\r\n AND `AddDate` > DATE_SUB(CURRENT_DATE, INTERVAL 6 DAY)\r\nGROUP BY DATE_FORMAT(`AddDate`, \'%Y%j\'), DATE_FORMAT(`AddDate`, \'%b %D (%a)\')\r\nORDER BY DATE_FORMAT(`AddDate`, \'%Y%j\') ASC LIMIT 7', '/admin/moneyStats', 'N'),
(8, 'Revenue by Currency Type', 80, 'Chart', 'Pie', 'danger-gradient', NULL, 'income by currency type', 'SELECT `CurrencyCode`, COALESCE(SUM(`GrossAmount`), 0) AS `Income`\r\nFROM `wtkRevenue`\r\nWHERE `PaymentStatus` IN (\'paid\',\'succeeded\',\'confirmed\',\'Authorized\')\r\n-- AND `AddDate` > (NOW() - INTERVAL 7 DAY)\r\nGROUP BY `CurrencyCode`\r\nORDER BY SUM(`GrossAmount`) DESC', '/admin/revenueList', 'N'),
(9, 'Activity 1 Day', 80, 'List', NULL, NULL, 'Y', 'Server access statistics', 'call make_widget(1)', NULL, 'N'),
(10, 'Activity 7 Days', 80, 'List', NULL, NULL, 'Y', 'Server access statistics', 'call make_widget(7)', NULL, 'N'),
(11, 'Activity 30 Days', 80, 'List', NULL, NULL, 'Y', 'Server access statistics', 'call make_widget(30)', NULL, 'N'),
(12, 'Update History', 80, 'Chart', 'Bar', NULL, NULL, 'data updates during last 5 days', 'SELECT DATE_FORMAT(`AddDate`, \'%b %D (%a)\') AS `Day`,\r\n    COUNT(`UID`) AS `Count`\r\nFROM `wtkUpdateLog`\r\nGROUP BY DATE_FORMAT(`AddDate`, \'%Y%j\'),DATE_FORMAT(`AddDate`, \'%b %D (%a)\')\r\nORDER BY DATE_FORMAT(`AddDate`, \'%Y%j\') DESC LIMIT 5', NULL, 'N'),
(13, 'Login History', 80, 'Chart', 'Bar', NULL, NULL, 'login activity last 5 days', 'SELECT DATE_FORMAT(`LastLogin`, \'%b %D (%a)\') AS `Day`,\r\n    COUNT(`UID`) AS `Count`\r\n  FROM `wtkLoginLog`\r\nwhere `LastLogin` IS NOT NULL\r\nGROUP BY DATE_FORMAT(`LastLogin`, \'%Y%j\'),DATE_FORMAT(`LastLogin`, \'%b %D (%a)\')\r\nORDER BY DATE_FORMAT(`LastLogin`, \'%Y%j\') DESC LIMIT 5', NULL, 'N'),
(14, 'Page View History', 80, 'Chart', 'Bar', NULL, NULL, 'page views during last 5 days', 'SELECT DATE_FORMAT(`AddDate`, \'%b %D (%a)\') AS `Day`,\r\n    COUNT(`UID`) AS `Count`\r\n  FROM `wtkUserHistory`\r\nGROUP BY DATE_FORMAT(`AddDate`, \'%Y%j\'),DATE_FORMAT(`AddDate`, \'%b %D (%a)\')\r\nORDER BY DATE_FORMAT(`AddDate`, \'%Y%j\') DESC LIMIT 5', NULL, 'N'),
(15, 'Errors Today', 99, 'Count', NULL, 'red-gradient', NULL, 'how many errors in last 24 hours', 'SELECT COUNT(`UID`)\r\n   FROM `wtkErrorLog`\r\n WHERE `DelDate` IS NULL AND `AddDate` > DATE_SUB(NOW(), INTERVAL 1 DAY)', 'errorLogList', 'N'),
(16, 'Active Users', 30, 'Count', 'Area', 'success-gradient', NULL, 'not including staff', 'SELECT COUNT(*) as `Count`\r\nFROM `wtkUsers`\r\nWHERE `DelDate` IS NULL AND `SecurityLevel` < 80', NULL, 'N'),
...
(22, 'Affiliates', 80, 'Count', NULL, 'success-gradient', NULL, NULL, 'SELECT COUNT(*)\r\n FROM `wtkAffiliates`\r\nWHERE `DelDate` IS NULL', '/admin/affiliateList', 'N'),
(23, 'Prospects', 80, 'Count', NULL, 'info-gradient', NULL, NULL, 'SELECT COUNT(*)\r\n FROM `wtkProspects`\r\nWHERE `DelDate` IS NULL', '/admin/prospectList', 'N'),
(24, 'Help System', 1, 'List', 'Area', 'danger-gradient', 'Y', NULL, 'SELECT `UID`, CONCAT(\'<a onclick=\"wtkShowHelp(\',`UID`,\')\">\',`HelpTitle`,\'</a>\') AS `HowTo`\r\n  FROM `wtkHelp`\r\nORDER BY `HelpTitle` ASC', NULL, 'N'),
(25, 'Revenue This Year', 80, 'Count', 'Area', 'success-gradient', 'N', 'income from the last 365 days', 'SELECT CONCAT(\'$\', FORMAT(COALESCE(SUM(`GrossAmount`), 0),0)) AS `Income`\r\nFROM `wtkRevenue`\r\nWHERE `AddDate` > (CURRENT_DATE - INTERVAL 365 DAY)\r\n  AND `PaymentStatus` IN (\'paid\',\'succeeded\',\'confirmed\',\'Authorized\')', '/admin/revenueList', 'N');
