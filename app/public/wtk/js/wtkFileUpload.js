...
function wtkSetProgress(e) {
    ...
    if (elementExist('photoProgressDIV' + pgFormId)) {
        if ($('#photoProgressDIV' + pgFormId).hasClass(pgHide)) {
            $('#photoProgressDIV' + pgFormId).removeClass(pgHide);
        }
...
//  console.log(e);
    ...
    if (elementExist('imgPreview' + pgFormId)) {
        ...
        if (elementExist('filePreview' + pgFormId)) {
            ...
            if ($('#filePreview' + pgFormId).hasClass(pgHide)) {
                $('#filePreview' + pgFormId).removeClass(pgHide);
            }
        }
    }
    ...
    if (pgFileSize < gloMaxFileSize){
        if (elementExist('wtkfPhoto')) { // used by wtkFileUpload
            if ($('#wtkfPhotoDIV').hasClass(pgHide)) {
                $('#wtkfPhotoDIV').removeClass(pgHide);
            }
            if ($('#wtkfSaveBtn').hasClass(pgHide)) {
                $('#wtkfSaveBtn').removeClass(pgHide);
            }
            if ($('#wtkf2btns').hasClass(pgHide)) {
                $('#wtkf2btns').removeClass(pgHide);
            }
        }
        if (elementExist('wtkfUploadBtn' + pgFormId)) {
            $('#wtkfUploadBtn' + pgFormId).removeClass(pgHide);
        }
    }
    if (elementExist('photoProgressDIV' + pgFormId)) {
            ...
            if (fncPastImg != '/wtk/imgs/noPhotoSmall.gif') {
                if (pgFileSize < gloMaxFileSize){
                    if ($('#wtkfDelBtn' + pgFormId).hasClass('hide')) {
                        ...
                        if ($('#wtkfDelBtn' + pgFormId).hasClass('hidden')) {
                            $('#wtkfDelBtn' + pgFormId).removeClass('hidden');
                            wtkDebugLog('wtkLoaded removeClass(hidden) for wtkfDelBtn' + pgFormId);
                        } else {
                            wtkDebugLog('wtkLoaded NOT removeClass(hidden) on wtkfDelBtn' + pgFormId);
                        }
                    }
                }
            } else {
...
function wtkfDelFile(fncId, fncFormId = '') {
    wtkDebugLog('wtkfDelFile: pgHide: ' + pgHide);
    $('#wtkfUploadBtn' + fncFormId).addClass(pgHide); // will work with MaterializeCSS and TailwindCSS
        ...
        success: function(data) {
            if (elementExist('imgPreview' + fncFormId)) {
                ...
                if (elementExist('filePreview' + fncFormId)) {
                    ...
                    fncShowFile.href = '';
                    $('#filePreview' + fncFormId).addClass(pgHide);
                }
            }
            $('#wtkfDelBtn' + fncFormId).addClass(pgHide);
            $('#wtkfAddBtn' + fncFormId).removeClass(pgHide);
            if (elementExist('wtkfPhoto')) {
                $('#wtkfPhotoDIV').addClass(pgHide);
                $('#wtkf2btns').removeClass(pgHide);
            }
...
function wtkfDelFileOld(fncFormId) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            $('#wtkfDelBtn').addClass(pgHide);
            $('#wtkfAddBtn').removeClass(pgHide);
            if (elementExist('wtkfPhoto')) {
                $('#wtkfPhotoDIV').addClass(pgHide);
                $('#wtkf2btns').removeClass(pgHide);
            }
...
function wtkfRevertImg() {
    ...
    if (fncPastImg == '/wtk/imgs/noPhotoSmall.gif') {
        ...
        $('#wtkf2btns').addClass(pgHide);
        $('#wtkfAddBtn').addClass(pgHide);
        $('#wtkfDelBtn').removeClass(pgHide);
    }
    pgFileToUpload = 'N';
} // wtkfRevertImg
...
function wtkShowImageUpload(fncId = ''){
    $('#uploadFileBtn' + fncId).addClass(pgHide);
    $('#uploadFileDIV' + fncId).removeClass(pgHide);
