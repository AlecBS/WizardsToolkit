...
$pgLabelActive = '';
$pgOtherHid = '';
$pgEmailCode = 'pet1'; // wtkGetParam('Mode');
...
SELECT `UID`, `Subject`
FROM `wtkEmailTemplate`
WHERE `EmailCode` LIKE 'pet%'
ORDER BY `UID` ASC
SQLVAR;
$pgSelOptions = wtkGetSelectOptions($pgSelSQL, [], 'Subject', 'UID', '4');
...
$pgSQL =<<<SQLVAR
SELECT p.`PetName` AS `ToName`, p.`OwnerEmail` AS `ToEmail`,
    CONCAT(COALESCE(u.`FirstName`,''), ' ', COALESCE(u.`LastName`,'')) AS `FromName`,
    u.`Email` AS `FromEmail`
  FROM `wtkUsers` u, `pets` p
 WHERE p.`UID` = :ToUID AND u.`UID` = :FromUID
SQLVAR;
$pgSqlFilter = array (
    'FromUID' => $gloUserUID,
    'ToUID' => $gloId
...
<div class="row">
    <div class="col s12"><br>
        ...
        &nbsp;&nbsp;
        <button id="sendEmailBtn" type="button" class="btn-primary btn-small b-shadow waves-effect waves-light modal-close" onclick="Javascript:alert('email disabled for demo')">Send</button>
        </span></h3>
        <br>
