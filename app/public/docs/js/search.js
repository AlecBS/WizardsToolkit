...
// BEGIN Wizards Toolkit documentation sidebar loading
function getSideNav(fncDir) {
    var xmlhttp = new XMLHttpRequest();
    xmlhttp.onreadystatechange = function() {
        if (xmlhttp.readyState == XMLHttpRequest.DONE) {   // XMLHttpRequest.DONE == 4
            if (xmlhttp.status == 200) {
                document.getElementById("sidenav").innerHTML = xmlhttp.responseText;
            } else if (xmlhttp.status == 400) {
                alert('There was a 400 error!');
            } else {
                alert('There was an error and side bar did not load.  Please contact support@programminglabs.com.');
            }
        }
    };
    if (fncDir == 'root') {
        xmlhttp.open("POST", "sidenav.php", true);
    } else {
        xmlhttp.open("POST", "../sidenav.php", true);
    }
    xmlhttp.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    xmlhttp.send('dir=' + fncDir);
}
//  END  Wizards Toolkit documentation sidebar loading

var pgWTKServer = false;
function wtkGoToURL(fncGoTo) {
    if (pgWTKServer == true) {
        alert('This link disabled on Wizard\'s Toolkit website but will work when you download WTK.');
    } else {
